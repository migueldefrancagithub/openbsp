...
export const CONVEX_ERROR_MESSAGES: Record<string, Pair> = {
  ...
  INVALID_STAGE_NAME: ["O nome da etapa tem de ter entre 2 e 50 caracteres.", "The stage name must be 2 to 50 characters."],
  INVALID_STAGE_COLOR: ["Escolha uma cor válida para a etapa.", "Choose a valid stage colour."],
  INVALID_STAGE_ORDER: ["A ordem das etapas mudou. Atualize a página e tente novamente.", "The stage order changed. Refresh the page and try again."],
  INVALID_STAGE_REDIRECT: ["Escolha outra etapa ativa para receber estes leads.", "Choose another active stage to receive these leads."],
  NEXT_STEP_REQUIRED: ["Esta etapa exige um próximo passo. Defina-o na conversa antes de mover.", "This stage requires a next step. Set it in the conversation before moving."],
  PIPELINE_ARCHIVED: ["Este funil já não está ativo.", "This pipeline is no longer active."],
  STAGE_ARCHIVED: ["Esta etapa já foi eliminada. Atualize o funil.", "This stage has been removed. Refresh the pipeline."],
  STAGE_LIMIT_REACHED: ["O funil atingiu o limite de 30 etapas.", "The pipeline reached its 30-stage limit."],
  ...
  AI_PROVIDER_NOT_CONFIGURED: ["Nenhuma chave configurada para este provedor de IA. Adicione a chave da clínica ou peça à plataforma.", "No key configured for this AI provider. Add the clinic's key or ask the platform."],
  AI_MODEL_REQUIRED: ["Indique um modelo válido (letras, números, pontos e hífens).", "Enter a valid model id (letters, digits, dots and dashes)."],
  AI_FALLBACK_SAME_PROVIDER: ["O provedor de recurso tem de ser diferente do principal.", "The fallback provider must differ from the primary one."],
  AI_BUDGET_INVALID: ["Orçamento diário inválido (0 a 1000 USD).", "Invalid daily budget (0 to 1000 USD)."],
  INVALID_API_KEY_FORMAT: ["A chave não parece válida para este provedor.", "The key does not look valid for this provider."],
  ...
  AI_AGENT_LIMIT: ["Limite de agentes atingido (50).", "Agent limit reached (50)."],
  AI_AGENT_NOT_PUBLISHABLE: ["O agente ainda tem bloqueios na lista de verificação. Resolva-os antes de publicar.", "The agent still has checklist blockers. Resolve them before publishing."],
  AI_AGENT_NOT_PUBLISHED: ["Publique o agente antes de o ativar ou pausar.", "Publish the agent before activating or pausing it."],
  AI_AGENT_INVALID_STATE: ["Só rascunhos nunca publicados podem ser apagados.", "Only never-published drafts can be deleted."],
  CHANNEL_REQUIRED: ["Escolha o canal WhatsApp do agente.", "Pick the agent's WhatsApp channel."],
  CHANNEL_NOT_SUPPORTED: ["O canal escolhido não é o canal Hub da clínica.", "The chosen channel is not the clinic's Hub channel."],
  CHANNEL_NOT_READY: ["O canal não está pronto para enviar.", "The channel is not ready to send."],
  KNOWLEDGE_REQUIRED: ["Selecione pelo menos um item de conhecimento ativo.", "Select at least one active knowledge item."],
  KNOWLEDGE_STALE: ["Há conhecimento com mais de 90 dias.", "Some knowledge is older than 90 days."],
  TOOL_REQUIRED: ["Falta uma ferramenta obrigatória para este objetivo.", "A required tool for this objective is missing."],
  TOOL_FORBIDDEN: ["Ferramenta não permitida para este objetivo.", "Tool not allowed for this objective."],
  TOOL_UNKNOWN: ["Ferramenta desconhecida na configuração.", "Unknown tool in the configuration."],
  FALLBACK_REQUIRED: ["Escreva a mensagem de recurso.", "Write the fallback message."],
  HANDOFF_MESSAGE_REQUIRED: ["Escreva a mensagem de passagem à equipa.", "Write the handoff message."],
  HANDOFF_CLINICAL_REQUIRED: ["Perguntas clínicas têm de passar à equipa.", "Clinical questions must hand off to the team."],
  INSTRUCTIONS_TOO_SHORT: ["As instruções são muito curtas.", "Instructions are very short."],
  INSTRUCTIONS_HEALTHCARE: ["As instruções contêm linguagem clínica.", "Instructions contain clinical language."],
  PROVIDER_NOT_CONFIGURED: ["Configure uma chave de IA em Definições › IA.", "Configure an AI key in Settings › AI."],
  PROVIDER_NOT_TESTED: ["Teste a ligação ao provedor em Definições › IA.", "Test the provider connection in Settings › AI."],
  BUDGET_REQUIRED: ["Defina um orçamento diário maior que zero.", "Set a daily budget above zero."],
  AGENT_CONFLICT: ["Já existe um agente ativo com este objetivo neste canal.", "Another active agent has this objective on this channel."],
  SANDBOX_NOT_RUN: ["Experimente o agente no sandbox antes de publicar.", "Try the agent in the sandbox before publishing."],
  DPIA_PROVIDER: ["Registe o provedor de IA como subcontratante na DPIA.", "Record the AI provider as a data processor in the DPIA."],
  TOOL_NOT_ALLOWED: ["O agente tentou usar uma ferramenta que não tem autorizada.", "The agent tried a tool it is not allowed to use."],
  TOOL_INPUT_INVALID: ["A ferramenta recebeu dados inválidos do modelo.", "The tool received invalid data from the model."],
  TOOL_FAILED: ["A ferramenta falhou.", "The tool failed."],
  SERVICE_NOT_FOUND: ["Serviço não encontrado.", "Service not found."],
  APPOINTMENT_NOT_FOUND: ["Não há marcação futura nesta conversa.", "There is no upcoming appointment on this conversation."],
  FOLLOW_UP_RULE_MISSING: ["Não existe regra de follow-up ativa para este gatilho.", "No active follow-up rule for this trigger."],
  TURN_NOT_FOUND: ["Turno de IA não encontrado.", "AI turn not found."],
  RUN_NOT_ACTIVE: ["A IA já não está ativa nesta conversa.", "The AI is no longer active on this conversation."],
  AGENT_NOT_FOUND: ["Agente não encontrado.", "Agent not found."],
  BUDGET_EXCEEDED: ["Orçamento diário de IA esgotado; os agentes retomam amanhã.", "Daily AI budget exhausted; agents resume tomorrow."],
  TURN_CAP: ["Limite de respostas automáticas atingido nesta conversa.", "Automatic reply limit reached on this conversation."],
  STALE_TURN: ["A IA não terminou a resposta a tempo.", "The AI did not finish the reply in time."],
  RUNTIME_ERROR: ["Erro interno do agente de IA.", "Internal AI agent error."],
  ...
  WEBHOOK_URL_INVALID: ["O URL do webhook tem de ser https público (sem localhost nem redes privadas).", "The webhook URL must be public https (no localhost or private networks)."],
  WEBHOOK_LIMIT: ["Limite de webhooks atingido (10).", "Webhook limit reached (10)."],
  WEBHOOK_EVENTS_REQUIRED: ["Escolha pelo menos um evento.", "Pick at least one event."],
  WEBHOOK_NOT_RETRYABLE: ["Esta entrega já não pode ser repetida.", "This delivery can no longer be retried."],
  API_KEY_INVALID: ["Chave de API inválida ou revogada.", "Invalid or revoked API key."],
  API_RATE_LIMITED: ["Demasiados pedidos; tente mais tarde.", "Too many requests; try again later."],
  AI_SUGGESTION_NOT_PENDING: ["Esta sugestão já foi tratada.", "This suggestion was already handled."],
  AI_MODE_REQUIRES_PUBLISH: ["Publique o agente antes de o pôr em co-piloto ou automático.", "Publish the agent before switching it to copilot or autopilot."],
  AI_NO_AGENT_FOR_CHANNEL: ["Não há agente publicado para este canal.", "No published agent for this channel."],
  DISCARDED: ["Sugestão descartada pela equipa.", "Suggestion discarded by the team."],
  HUMAN_REPLIED: ["A equipa respondeu antes de aprovar a sugestão.", "The team replied before approving the suggestion."],
  AGENT_SANDBOX: ["O agente está em modo sandbox e não responde a conversas reais.", "The agent is in sandbox mode and does not answer real conversations."],
  ...
  DECISION_NOT_PENDING: ["Esta decisão já foi tratada.", "This decision has already been handled."],
  DECISION_ACTION_NOT_ALLOWED: ["A decisão inclui uma ação que a rotina não autoriza.", "The decision includes an action the routine does not allow."],
  ROUTINE_NOT_ACTIVE: ["Esta rotina não está ativa.", "This routine is not active."],
  ROUTINE_CUSTOMER_SEND_NOT_READY: ["O envio automático ao cliente ainda não está autorizado para esta rotina.", "Automatic customer messaging is not yet authorised for this routine."],
  STAFF_PHONE_REQUIRED: ["Adicione um número WhatsApp válido para esta pessoa da equipa.", "Add a valid WhatsApp number for this team member."],
  INVALID_PHONE: ["Adicione um número em formato internacional válido.", "Add a valid international phone number."],
  STAFF_NOTIFICATION_URL_INVALID: ["O endpoint UAZAPI tem de ser um URL https público.", "The UAZAPI endpoint must be a public https URL."],
  STAFF_NOTIFICATION_TOKEN_REQUIRED: ["Introduza o token UAZAPI para guardar a ligação.", "Enter the UAZAPI token to save the connection."],
  STAFF_NOTIFICATION_TOKEN_INVALID: ["O token UAZAPI não tem um formato válido.", "The UAZAPI token does not have a valid format."],
  UAZ_CONNECTION_NOT_FOUND: ["A ligação de notificações internas não está configurada.", "The internal notification connection is not configured."],
  UAZ_CONNECTION_PAUSED: ["A ligação de notificações internas está pausada. Verifique o token e tente novamente.", "The internal notification connection is paused. Check the token and try again."],
  UAZ_TOKEN_REQUIRED: ["Introduza o token UAZAPI para guardar a ligação.", "Enter the UAZAPI token to save the connection."],
  STAFF_NOTIFICATION_NOT_RETRYABLE: ["Esta notificação interna já não pode ser repetida.", "This internal notification can no longer be retried."],
