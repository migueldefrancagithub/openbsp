    ...
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="flex items-center gap-2 border-b border-slate-100 px-6 py-4">
          <CalendarCog size={16} className="text-[#0a1b33]" />
          <div>
            <h2 className="text-[15px] font-semibold text-[#0a1b33]">{tr("Agenda e notificações", "Calendar and notifications")}</h2>
            <p className="text-xs text-slate-500">{tr("Fuso horário, passo dos horários, antecedência mínima e mensagens de confirmação/lembrete.", "Time zone, slot step, minimum lead time and confirmation/reminder messages.")}</p>
          </div>
        </div>
        {!form ? (
          <div className="px-6 py-6"><Loader2 size={15} className="animate-spin text-slate-300" /></div>
        ) : (
          <div className="space-y-4 p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block text-[11px] font-medium text-slate-500">
                ...
                <textarea rows={3} value={form.confirmationText} onChange={(e) => setForm({ ...form, confirmationText: e.target.value })} placeholder={tr("Olá {{nome}}, a sua consulta de {{servico}} está marcada para {{quando}} na {{clinica}}. Responda CONFIRMO para confirmar.", "Hi {{nome}}, your {{servico}} appointment is on {{quando}} at {{clinica}}. Reply CONFIRMO to confirm.")} className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-[#0a1b33] outline-none focus:border-slate-400" />
              </label>
              <label className="block text-[11px] font-medium text-slate-500">
                {tr("Texto de lembrete", "Reminder text")}
                <textarea rows={3} value={form.reminderText} onChange={(e) => setForm({ ...form, reminderText: e.target.value })} placeholder={tr("Olá {{nome}}, lembramos a sua consulta de {{servico}} {{quando}}.", "Hi {{nome}}, a reminder of your {{servico}} appointment {{quando}}.")} className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-[#0a1b33] outline-none focus:border-slate-400" />
              </label>
            </div>
            <p className="text-[11px] text-slate-500">{tr("Variáveis: {{nome}}, {{servico}}, {{quando}}, {{clinica}}.", "Variables: {{nome}}, {{servico}}, {{quando}}, {{clinica}}.")}</p>
            ...
            <div className="flex justify-end">
              <button type="button" disabled={busy !== null} onClick={() => void submitSettings()} className="inline-flex h-10 items-center gap-2 rounded-lg bg-[#0a1b33] px-4 text-[13px] font-semibold text-white disabled:opacity-50">
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="flex flex-col gap-3 border-b border-slate-100 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            <Stethoscope size={16} className="text-[#0a1b33]" />
            <div>
              <h2 className="text-[15px] font-semibold text-[#0a1b33]">{tr("Profissionais", "Professionals")}</h2>
              <p className="text-xs text-slate-500">{tr("Cada profissional tem a sua agenda; sem profissionais a agenda é por serviço.", "Each professional has their own calendar; without professionals the calendar is per service.")}</p>
            </div>
          </div>
          <button type="button" onClick={() => setEditing({ name: "", specialty: "", color: COLORS[(professionals?.length ?? 0) % COLORS.length], days: [1, 2, 3, 4, 5], start: "08:00", end: "17:00", serviceIds: [] })} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 px-3 text-[12px] font-semibold text-[#0a1b33]">
        ...
        <div className="p-6">
          {professionals === undefined ? (
            ...
            <p className="text-[13px] text-slate-500">{tr("Ainda sem profissionais.", "No professionals yet.")}</p>
          ) : (
                ...
                <li key={p._id} className="flex items-center justify-between gap-3 py-2">
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <div className="truncate text-[13px] font-semibold text-[#0a1b33]">{p.name}</div>
                      <div className="text-[11px] text-slate-500">{p.specialty ?? tr("Sem especialidade", "No specialty")}{p.availability?.length ? ` · ${p.availability.length} ${tr("dias", "days")}` : ""}</div>
                    </div>
                  </div>
                  <div className="flex shrink-0 gap-1.5">
                    <button type="button" onClick={() => setEditing({ professionalId: p._id, name: p.name, specialty: p.specialty ?? "", color: p.color ?? COLORS[0], days: p.availability?.map((a) => a.weekday) ?? [], start: p.availability?.[0]?.start ?? "08:00", end: p.availability?.[0]?.end ?? "17:00", serviceIds: (workspace?.services ?? []).filter((s) => s.professionalIds?.includes(p._id)).map((s) => s._id) })} className="h-8 rounded-md border border-slate-200 px-2 text-[11px] font-semibold text-slate-600">{tr("Editar", "Edit")}</button>
                    <button type="button" disabled={busy !== null} onClick={() => { if (window.confirm(tr("Arquivar este profissional?", "Archive this professional?"))) void run(`arch-${p._id}`, () => archiveProfessional({ professionalId: p._id })); }} className="inline-flex h-8 items-center gap-1 rounded-md border border-slate-200 px-2 text-[11px] font-semibold text-slate-500 hover:text-[#b3261e]"><Archive size={12} /></button>
                  </div>
                </li>
              ))}
            </ul>
          ...
          {editing && (
            <div className="mt-4 space-y-3 rounded-lg border border-slate-200 bg-slate-50 p-4">
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <div className="block text-[11px] font-medium text-slate-500">
                  ...
                  <div className="mt-2 flex gap-1.5">
                    {COLORS.map((c) => (
                      <button key={c} type="button" onClick={() => setEditing({ ...editing, color: c })} className={cn("h-6 w-6 rounded-full border-2", editing.color === c ? "border-[#0a1b33]" : "border-transparent")} style={{ backgroundColor: c }} aria-label={c} />
                    ))}
                  </div>
                </div>
              </div>
                ...
                <div className="text-[11px] font-medium text-slate-500">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    {weekdays.map((label, weekday) => {
                      ...
                      return (
                        <button key={weekday} type="button" onClick={() => setEditing({ ...editing, days: on ? editing.days.filter((d) => d !== weekday) : [...editing.days, weekday].sort() })} className={cn("rounded-md border px-2 py-1 text-[11px] font-semibold", on ? "border-[#0a1b33] bg-[#0a1b33] text-white" : "border-slate-200 bg-white text-slate-600")}>{label}</button>
                      );
                    })}
                  </div>
                </div>
                <label className="block text-[11px] font-medium text-slate-500">{tr("Início", "Start")}<input type="time" value={editing.start} onChange={(e) => setEditing({ ...editing, start: e.target.value })} className={inputClass} /></label>
                <label className="block text-[11px] font-medium text-slate-500">{tr("Fim", "End")}<input type="time" value={editing.end} onChange={(e) => setEditing({ ...editing, end: e.target.value })} className={inputClass} /></label>
              </div>
                ...
                <div className="text-[11px] font-medium text-slate-500">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    {(workspace?.services ?? []).filter((s) => s.status === "active").map((s) => {
                      ...
                      return (
                        <button key={s._id} type="button" onClick={() => setEditing({ ...editing, serviceIds: on ? editing.serviceIds.filter((id) => id !== s._id) : [...editing.serviceIds, s._id] })} className={cn("rounded-full border px-2.5 py-1 text-[11px] font-semibold", on ? "border-[#0d6b61] bg-[#0d6b61] text-white" : "border-slate-200 bg-white text-slate-600")}>{s.name}</button>
                      );
                    })}
                  </div>
                </div>
              ...
              <div className="flex justify-end gap-2">
                ...
                <button type="button" disabled={busy !== null || editing.name.trim().length < 2} onClick={() => void submitProfessional()} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-[#0a1b33] px-3 text-[12px] font-semibold text-white disabled:opacity-50">
