    ...
    <section className="overflow-hidden rounded-lg border border-line bg-surface">
      <div className="flex flex-col gap-3 border-b border-line-soft px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          <Route size={16} className="text-ink" />
          <div>
            <h2 className="text-[15px] font-semibold text-ink">{tr("Atribuição automática", "Automatic assignment")}</h2>
            <p className="text-xs text-muted">{tr("Novas conversas recebem um responsável da equipa: rotativo ou quem tem menos conversas abertas.", "New conversations get an owner from the team: round-robin or whoever has fewer open conversations.")}</p>
          </div>
        </div>
        <button
          type="button"
          ...
          className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-line px-3 text-[12px] font-semibold text-ink disabled:opacity-50"
        >
      ...
      <div className="p-6">
        ...
        {rules === undefined ? (
          ...
          (teams ?? []).length > 0 && !editing && <p className="text-[13px] text-muted">{tr("Sem regras: as conversas ficam por atribuir até alguém as assumir.", "No rules: conversations stay unassigned until someone takes them.")}</p>
        ) : (
              ...
              <li key={rule._id} className="flex items-center justify-between gap-3 py-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 text-[13px] font-semibold text-ink">
                    <span className={cn("h-2 w-2 rounded-full", rule.active ? "bg-[#0d6b61]" : "bg-faint/50")} />
        ...
        {editing && (
          <div className="mt-4 space-y-3 rounded-lg border border-line bg-surface-2 p-4">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <label className="block text-[11px] font-medium text-muted">{tr("Nome", "Name")}<input value={editing.name} onChange={(e) => setEditing({ ...editing, name: e.target.value })} className={inputClass} maxLength={80} /></label>
              <label className="block text-[11px] font-medium text-muted">{tr("Equipa", "Team")}
              ...
              <label className="block text-[11px] font-medium text-muted">{tr("Estratégia", "Strategy")}
            ...
            <div className="flex justify-end gap-2">
              ...
              <button type="button" disabled={busy || editing.name.trim().length < 2 || !editing.teamId} onClick={() => void submit()} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-brand-solid px-3 text-[12px] font-semibold text-white disabled:opacity-50">{busy && <Loader2 size={12} className="animate-spin" />} {tr("Guardar", "Save")}</button>
            </div>
          </div>
        )}
      </div>
