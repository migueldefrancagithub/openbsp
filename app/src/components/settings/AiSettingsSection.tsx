    ...
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="flex items-center gap-2 border-b border-slate-100 px-6 py-4">
          <Bot size={16} className="text-[#0a1b33]" />
          <div>
            <h2 className="text-[15px] font-semibold text-[#0a1b33]">{tr("Inteligência artificial", "Artificial intelligence")}</h2>
            <p className="text-xs text-slate-500">{tr("Provedor, modelos e limites usados pelos agentes desta clínica. A chave nunca é mostrada depois de guardada.", "Provider, models and limits used by this clinic's agents. The key is never shown again after saving.")}</p>
          </div>
          {settings && (
          ...
          <div className="space-y-4 p-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex gap-2">
                <button type="button" disabled={busy !== null} onClick={() => void run("probe", () => probe({}), tr("Provedor respondeu.", "Provider responded."))} className="inline-flex h-10 items-center gap-2 rounded-lg border border-slate-200 px-4 text-[13px] font-semibold text-[#0a1b33] disabled:opacity-50">
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="flex items-center gap-2 border-b border-slate-100 px-6 py-4">
          <KeyRound size={16} className="text-[#0a1b33]" />
          <div>
            <h2 className="text-[15px] font-semibold text-[#0a1b33]">{tr("Chaves de API", "API keys")}</h2>
            <p className="text-xs text-slate-500">{tr("Opcional: chave própria da clínica por provedor (encriptada). Sem chave própria usa-se a da plataforma quando existir.", "Optional: the clinic's own key per provider (encrypted). Without one, the platform key is used when available.")}</p>
          </div>
        </div>
        <div className="space-y-3 p-6">
          <ul className="grid gap-2 sm:grid-cols-3">
            {PROVIDERS.map((p) => {
              ...
              return (
                <li key={p.id} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[12px]">
                  <div className="font-semibold text-[#0a1b33]">{p.label}</div>
          ...
          <div className="grid gap-2 sm:grid-cols-[180px_1fr_auto]">
            <select value={keyInput.provider} onChange={(e) => setKeyInput({ ...keyInput, provider: e.target.value as Provider })} className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm text-[#0a1b33] outline-none">
              {PROVIDERS.map((p) => <option key={p.id} value={p.id}>{p.label}</option>)}
            </select>
            <input type="password" autoComplete="off" value={keyInput.value} onChange={(e) => setKeyInput({ ...keyInput, value: e.target.value })} placeholder={tr("Colar a chave (guardada encriptada)", "Paste the key (stored encrypted)")} className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm text-[#0a1b33] outline-none" />
            <button
              type="button"
              ...
              className="h-10 rounded-lg border border-slate-200 px-4 text-[13px] font-semibold text-[#0a1b33] disabled:opacity-50"
            >
