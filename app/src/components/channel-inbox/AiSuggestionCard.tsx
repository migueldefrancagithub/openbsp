...
export function AiSuggestionCard({ threadId, onUseDraft, windowOpen }: { threadId: Id<"channelThreads">; onUseDraft: (text: string) => void; windowOpen: boolean }) {
  ...
  async function run(key: string, action: () => Promise<unknown>) {
    ...
    try {
      await action();
      if (key === "approve") {
        // The card leaves with a flash instead of blinking out: the operator
        // needs to see that their approval turned into a sent message.
        setSent(true);
        globalThis.setTimeout(() => setSent(false), 700);
      }
    } catch (err) {
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        <button
          ...
          onClick={() => void run("approve", () => approve({ turnId: pending.turnId, text, approvedActionIndexes: selected }))}
          className={cn(
            "inline-flex h-8 items-center gap-1.5 rounded-md px-3 text-[11px] font-bold text-white transition-transform active:scale-[0.97] disabled:opacity-50",
            edited ? "bg-[#2b4f8a]" : "bg-[#0d6b61]",
          )}
        >
          {busy === "approve" ? <Loader2 size={12} className="animate-spin" /> : <Check size={12} />}
          {edited ? tr("Aprovar editada e enviar", "Approve edited and send") : tr("Aprovar e enviar", "Approve and send")}
          <kbd className="ml-0.5 rounded bg-white/20 px-1 py-0.5 font-mono text-[9px] font-semibold">⌘↵</kbd>
        </button>
