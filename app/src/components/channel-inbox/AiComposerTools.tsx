...
export function AiComposerTools({ threadId, draft, onUse, disabled }: { threadId: Id<"channelThreads">; draft: string; onUse: (text: string) => void; disabled?: boolean }) {
  ...
  const button = "inline-flex h-7 items-center gap-1 rounded-md border border-line bg-surface px-2 text-[11px] font-semibold text-body hover:border-[#2b4f8a] hover:text-chip-info-fg disabled:opacity-50";
  ...
  return (
    <div className="space-y-1.5" data-ai-composer>
      ...
      {result && (
        <div className={cn("rounded-lg border px-3 py-2", result.flagged.length > 0 ? "border-[#e0533d]/40 bg-chip-danger" : "border-chip-warn-fg/25 bg-chip-warn")}>
          <div className="mb-1 flex items-center justify-between text-[10px] font-semibold uppercase tracking-[0.12em] text-chip-warn-fg">
          ...
          {result.flagged.length > 0 && <p className="mt-1 text-[10px] font-semibold text-chip-danger-fg">{tr("Atenção: ", "Warning: ")}{result.flagged.map((f) => f.split(":")[0]).join(", ")} — {tr("corrija antes de enviar.", "fix before sending.")}</p>}
        </div>
      )}
    </div>
  );
