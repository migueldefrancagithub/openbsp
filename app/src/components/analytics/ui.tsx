...
import { NO_DATA, TABS, type TabId } from "./lib";
...
export function Module({
  ...
  return (
    <section
      className={cn(
        "min-w-0 rounded-xl border border-slate-200/80 bg-white",
        className,
      )}
    >
      {(title || action) && (
...
export function RiskBadge({
  ...
  if (!hasTraffic) {
    return (
      <span className="text-[11px] font-medium text-slate-400">No data</span>
    );
  }
  const tone =
    risk === "low"
  ...
  return (
    <span className={cn("inline-flex items-center gap-1.5 text-[11px] font-medium", tone)}>
      ...
      {risk === "low" ? "Healthy" : risk === "watch" ? "Watch" : "At risk"}
    </span>
  );
}
...
export function KpiStrip({
  ...
  return (
    <div className="grid min-w-0 grid-cols-[repeat(auto-fit,minmax(0,1fr))] divide-y divide-slate-100 overflow-hidden rounded-xl border border-slate-200/80 bg-white @2xl:grid-cols-4 @2xl:divide-x @2xl:divide-y-0 @md:grid-cols-2">
      {items.map((item) => (
...
export function Tabs({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className="flex min-w-0 gap-1 overflow-x-auto border-b border-slate-200 px-4 sm:px-6"
    >
      {TABS.map((tab) => {
        ...
        return (
          <button
            ...
            className={cn(
              "-mb-px shrink-0 border-b-2 px-3 py-2 text-[13px] font-medium transition-colors outline-none",
  ...
  return (
    <div className="flex min-h-[60vh] min-w-0 flex-col items-center justify-center px-6 text-center">
      ...
      <p className="mt-1.5 max-w-sm text-sm leading-relaxed text-slate-500">
        Analytics fill in once a connected channel starts sending and receiving.
...
export function EmptyRow({ label = "No data for this period" }: { label?: string }) {
  return (
