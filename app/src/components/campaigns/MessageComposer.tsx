  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1.2fr_1fr]">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active ? "border-brand-solid bg-brand-solid text-white" : "border-line bg-surface hover:border-line",
                )}
              >
                <span className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-md", active ? "bg-white/10" : "bg-surface-3 text-body")}>
                  <Icon size={15} />
                </span>
                <span className="min-w-0">
                  <span className="block text-[13px] font-semibold">{option.title}</span>
                  <span className={cn("block text-[11px]", active ? "text-white/80" : "text-muted")}>{option.desc}</span>
                </span>
              </button>
            );
          })}
        ...
        {draft.kind === "channel_template" ? (
          <>
            <label className="block">
              <span className="text-[11px] font-medium text-muted">{tr("Template", "Template")}</span>
            ...
            {variableCount > 0 && (
              <div className="space-y-2">
                <div className="text-[11px] font-medium uppercase tracking-[0.14em] text-faint">{tr("Variáveis", "Variables")}</div>
                {draft.bindings.map((binding) => (
                  <div key={binding.index} className="grid gap-2 sm:grid-cols-[80px_170px_1fr]">
                    <div className="flex h-10 items-center rounded-lg border border-line bg-surface px-3 font-mono text-[12px] text-muted">{`{{${binding.index}}}`}</div>
                    <select
          ...
          <label className="block">
            <span className="text-[11px] font-medium text-muted">{tr("Mensagem", "Message")}</span>
            <textarea
              value={draft.text}
              ...
              className="mt-1 w-full rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink outline-none focus:border-brand-solid/40"
            />
      ...
      <aside className="rounded-lg border border-line bg-surface p-4">
        <div className="mb-3 text-[13px] font-semibold text-ink">{tr("Pré-visualização", "Preview")}</div>
        <WhatsAppIosPreview
